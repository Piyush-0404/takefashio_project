...
export async function GET() {
  ...
  try { const cart = await db.cart.findUnique({ where: { userId: user.id }, include: { items: { include: { product: true }, orderBy: { id: "asc" } } } }); return ok({ cart: cart ?? { items: [] } }); } catch (error) { return errorResponse(error); }
}
